...
<form id="identity-form"><h2>Check registration</h2><label>Full Name<input name="fullName" required></label><label>Contact Number<input name="contact" inputmode="tel" pattern="[0-9]{10}" maxlength="10" placeholder="10-digit mobile number" required></label><p class="step-note">Full Name and Contact Number must both be unique. You cannot fill the form if either has already been registered.</p><button class="submit" type="submit">Continue</button><p class="message" id="identity-message" role="status"></p></form>
...
identityForm.addEventListener('submit', async (event) => {
  ...
  const button = identityForm.querySelector('button'); button.disabled = true; button.textContent = 'Checking…'; identityMessage.textContent = '';
  try { await request('check', identity); registrationForm.elements.fullName.value = identity.fullName; registrationForm.elements.contact.value = identity.contact; identityForm.hidden = true; registrationForm.hidden = false; registrationForm.elements.grade.focus(); }
  catch (error) { identityMessage.textContent = error.message; button.disabled = false; button.textContent = 'Continue'; }
});
